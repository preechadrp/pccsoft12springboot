...
public class ManAcApRem {
	...
	public static void getReport(LoginBean _loginBean, java.sql.Date toPostdate, String cust_cde, String fromAcctid,
		...
		try (FDbc dbc = FDbc.connectMasterDb()) {
			...
			reportParams.put("reportName", "รายงานเจ้าหนี้คงเหลือรายตัว");
	...
	public static FJRBeanCollectionDataSource createDataSource(FDbc dbc, LoginBean _loginBean, java.sql.Date toPostdate,
		...
		sql.addLine(" from " + TboACGL_AP.tablename);
		...
		sql.addLine(") aa1");
		sql.addLine("left join " + TboACGL_AP.tablename + " aa2 on aa1.COMP_CDE=aa2.COMP_CDE and aa1.CUST_CDE=aa2.CUST_CDE and aa1.LINK_NO=aa2.LINK_NO and aa1.DOCNO=aa2.DOCNO and aa2.POST_TYPE=1 ");
		sql.addLine("left join " + TboACCT_NO.tablename + " bb2 on aa2.COMP_CDE=bb2.COMP_CDE and aa2.ACCT_ID=bb2.ACCT_ID");
		sql.addLine("left join " + TboFCUS.tablename + " cus on aa2.COMP_CDE=cus.COMP_CDE and aa2.CUST_CDE=cus.CUST_CDE and COALESCE(aa2.CUST_CDE,'') != '' ");
	...
	public static JasperReportBuilder genReportPDF(FJRBeanCollectionDataSource dataSrc, Map reportParams,
		...
		CustomGroupBuilder acctGroup = DynamicReports.grp.group(new AbstractSimpleExpression() {
			...
			public String evaluate(ReportParameters reportParameters) {
				...
				String ret = "รหัสเจ้าหนี้ " + s1 + " " + s2 + " " + s3 + " " + s4;
	...
	public static JasperReportBuilder genReportExcel(JRDataSource dataSrc, Map reportParams, LoginBean _loginBean) {
		...
		myreport.addColumn(DynamicReports.col.column("รหัสเจ้าหนี้", new AbstractSimpleExpression<String>() {
			@Override
			public String evaluate(ReportParameters reportParameters) {
		...
		myreport.addColumn(DynamicReports.col.column("ชื่อเจ้าหนี้", new AbstractSimpleExpression<String>() {
			@Override
			public String evaluate(ReportParameters reportParameters) {
