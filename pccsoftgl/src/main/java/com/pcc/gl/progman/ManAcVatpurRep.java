...
public class ManAcVatpurRep {
	...
	public static void getReport(LoginBean _loginBean, java.sql.Date fromPostdate, java.sql.Date toPostdate,
		...
		try (FDbc dbc = FDbc.connectMasterDb()) {
			...
			reportParams.put("reportName", "รายงานภาษีซื้อ");
			...
			FJRBeanCollectionDataSource reportDataSource = createDataSource(dbc, _loginBean, fromPostdate, toPostdate, print_option);
	...
	public static FJRBeanCollectionDataSource createDataSource(FDbc dbc, LoginBean _loginBean,
		...
		sql.addLine("where aa.COMP_CDE='" + Fnc.sqlQuote(_loginBean.getCOMP_CDE()) + "' and aa.TAX_TYPE=2");
	...
	public static JasperReportBuilder genReportPDF(FJRBeanCollectionDataSource dataSrc, Map reportParams,
		...
		myreport.addColumn(DynamicReports.col.column("ผู้ขาย/ผู้ให้บริการ", new AbstractSimpleExpression<String>() {
			@Override
			public String evaluate(ReportParameters reportParameters) {
