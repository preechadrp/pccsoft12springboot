...
public class ManAcBala {
	...
	public static void getReport(LoginBean _loginBean, java.sql.Date toPostdate, FJasperPrintList fJasperPrintList,
		...
		try (FDbc dbc = FDbc.connectMasterDb()) {
			...
			FJRBeanCollectionDataSource reportDataSource = createDataSource(dbc, _loginBean, toPostdate, print_option);
	...
	public static FJRBeanCollectionDataSource createDataSource(FDbc dbc, LoginBean _loginBean, java.sql.Date toPostdate,
			int print_option) throws SQLException, Exception {

		java.sql.Date glFrmDate = TbmFCOMP.getGldate(dbc, _loginBean.getCOMP_CDE(), toPostdate);
		...
		sql.addLine("and dd.postdate between '" + glFrmDate + "' and '" + toPostdate + "' ");
		sql.addLine("and att.ACCT_TYPE in (1,2,3)");
	...
	public static JasperReportBuilder genReportPDF(FJRBeanCollectionDataSource dataSrc, Map reportParams,
		...
		JasperReportBuilder myreport = DynamicReports.report()
				...
				.columnHeader(DynamicReports.cmp.verticalGap(2));// trick แก้ปํญหากรณี pdf แสดงสีแถวแบบ highlight
		...
		VariableBuilder<BigDecimal> varSUM_GROUP = DynamicReports.variable("varSUM_GROUP",
				new AbstractSimpleExpression<BigDecimal>() {
					...
					public BigDecimal evaluate(ReportParameters reportParameters) {
						...
						if (acct_type.intValue() == 2 || acct_type.intValue() == 3) {// หนี้สิน , ทุน
							ret = ret.negate();
						}
						return ret;
					}
		...
		VariableBuilder<String> varLABEL_GROUP = DynamicReports.variable("varLABEL_GROUP",
				new AbstractSimpleExpression<String>() {
					...
					public String evaluate(ReportParameters reportParameters) {
						Integer acct_type = (Integer) reportParameters.getFieldValue("ACCT_TYPE");
						if (acct_type.intValue() == 1) {// สินทรัพย์
							return "รวมสินทรัพย์ :";
						} else if (acct_type.intValue() == 2) {// หนี้สิน
							return "รวมหนี้สิน :";
						} else {
							return "รวมทุน :";
						}
		...
		myreport.addColumn(DynamicReports.col.column("ยอดคงเหลือ", new AbstractSimpleExpression<BigDecimal>() {
			...
			public BigDecimal evaluate(ReportParameters reportParameters) {
				Integer acct_type = (Integer) reportParameters.getFieldValue("ACCT_TYPE");
				BigDecimal ret = (BigDecimal) reportParameters.getFieldValue("AMT");
				if (acct_type.intValue() == 2 || acct_type.intValue() == 3) {
	...
	public static JasperReportBuilder genReportExcel(JRDataSource dataSrc, Map reportParams, LoginBean _loginBean) {
		...
		}).setPadding(0);// trick ทำให้ Excel ตรง group header ไม่เยื้องกันระหว่าง detail;
		myreport.groupBy(acctGroup);
		// == end group varible
		...
		varSUM_GROUP.setResetType(Evaluation.GROUP);
		varSUM_GROUP.setResetGroup(acctGroup);// trick reset type group
		myreport.addVariable(varSUM_GROUP);
	...
	public static void getTestReport(LoginBean _loginBean, java.sql.Date toPostdate, FJasperPrintList fJasperPrintList,
		...
		try (FDbc dbc = FDbc.connectMasterDb()) {
			...
			String sql = " SELECT * FROM ACGL_DETAIL WHERE COMP_CDE = 'M03' AND VOU_TYPE = 'BK' AND VOU_NO = '5905000001' ";
			java.sql.ResultSet rs = dbc.getResultSet(sql);
			fJasperPrintList.addJasperPrintList(
					FReport.builtRepByResultSet("/pcc/jasper/ac/acBala/TestBal.jrxml", reportParams, rs));
