package com.pcc.gl.ui.acVatpurRep;

import org.zkoss.zul.Button;
import org.zkoss.zul.Datebox;

import com.pcc.gl.progman.ManAcVatpurRep;
...
public class AcVatpurRep extends FWinMenu {
	...
	public void onClick_btnPrint(int print_option) {
		...
		try {
			...
			ManAcVatpurRep.getReport(this.getLoginBean(), FnDate.getSqlDate(tdbFromPostdate.getValue()),
			...
			if (fJasperPrintList.getJasperPrintLst().size() > 0) {
				if (print_option == 1) {
					...
					FReport.exportExcel(fJasperPrintList, "AcVatpurRep");
				}
			} else {
