...
public class AcglVatpur extends FWinUtil {
		...
		try {
			...
			try (FDbc dbc = FDbc.connectMasterDb()) {
				...
				if (save_option == 1) {
					...
					boolean old_rec = TbfACGL_VATPUR.find(dbc, vatpur);

					if (this.getTax_type() == 1) {// 1=ภาษีซื้อรอตัด
						if (Fnc.getBigDecimal(vatpur.getCLEAR_AMT()).compareTo(BigDecimal.ZERO) != 0) {
							throw new Exception("รายการมีการทำรับภาษีซื้อแล้ว ไม่อนุญาติให้แก้ไข/ลบ");
						}
					}

					vatpur.setPOSTDATE(this.getAcgl_detail().getPOSTDATE());
					vatpur.setTAX_TYPE(this.getTax_type());// 1=ภาษีซื้อรอตัด ,2=ภาษีซื้อ
					vatpur.setPOST_TYPE(1);// 1=ขาตั้งหนี้,2=ขาล้างหนี้
					vatpur.setVAT_RATE(decVAT_RATE.getValue());
					...
					vatpur.setAMT(this.getAcgl_detail().getAMT());
					vatpur.setBASE_AMT(decBASE_AMT.getValue());
					vatpur.setCLEAR_AMT(BigDecimal.ZERO);
					vatpur.setSECT_ID(this.getAcgl_detail().getSECT_ID());
					vatpur.setDESCR(this.getAcgl_detail().getDESCR());
					vatpur.setDOCNO(txtDOCNO.getValue());
					vatpur.setDOCDATE(FnDate.getSqlDate(tdbDOCDATE.getValue()));
					vatpur.setCUST_CDE(txtCUST_CDE.getValue());
					vatpur.setCUST_BRANCH_ID(txtCUST_BRANCH_ID.getValue());
					if (Fnc.isEmpty(vatpur.getLINK_NO())) {
						vatpur.setLINK_NO(RunningAc.get_LINK_NO(dbc, this.getLoginBean()));
					}
					vatpur.setRECSTA(0);// 0=ยังไม่บันทึก,1=ยังไม่อนุมัติ,2=อนุมัติ,9=ยกเลิก'

					vatpur.setUPBY(this.getLoginBean().getUSER_ID());
					vatpur.setUPDT(FnDate.getTodaySqlDateTime());

					if (!old_rec) {
						vatpur.setINSBY(this.getLoginBean().getUSER_ID());
						vatpur.setINSDT(FnDate.getTodaySqlDateTime());
						TbfACGL_VATPUR.insert(dbc, vatpur);
					} else {
						TbfACGL_VATPUR.update(dbc, vatpur, "");
					}
					...
					if (TbfACGL_VATPUR.find(dbc, vatpur)) {
						if (this.getTax_type() == 1) {// 1=ภาษีซื้อรอตัด
							if (Fnc.getBigDecimal(vatpur.getCLEAR_AMT()).compareTo(BigDecimal.ZERO) != 0) {
								throw new Exception("รายการมีการทำรับภาษีซื้อแล้ว ไม่อนุญาติให้แก้ไข/ลบ");
							}
				...
				ManAcEntr.deleteAll_VOU_SEQ(dbc, this.getAcgl_detail(), "acgl_vatpur");
