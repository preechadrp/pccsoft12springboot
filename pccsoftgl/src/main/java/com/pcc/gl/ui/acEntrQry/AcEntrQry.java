...
public class AcEntrQry extends FWinMenu {
	...
	public void setFormObj() {
		...
		cmbRECSTA = (Combobox) getFellow("cmbRECSTA");
	...
	public void formInit() {
		try {
			...
			ZkUtil.appendItemToComboBox(cmbRECSTA, "", "ทั้งหมด");
			ZkUtil.appendItemToComboBox(cmbRECSTA, "0", "ยังไม่บันทึก");
			ZkUtil.appendItemToComboBox(cmbRECSTA, "1", "ยังไม่อนุมัติ");
			ZkUtil.appendItemToComboBox(cmbRECSTA, "2", "อนุมัติ");
			ZkUtil.appendItemToComboBox(cmbRECSTA, "9", "ยกเลิก");
			cmbRECSTA.setSelectedIndex(0);
	...
	public void onClick_btnFind() {
		try {
			...
			TbmACGL_HEADER.getDataQry(lst_gl_header, txtVOU_TYPE.getValue(), txtVOU_NO.getValue(),
					FnDate.getSqlDate(tdbPOSTDATE_FROM.getValue()), FnDate.getSqlDate(tdbPOSTDATE_TO.getValue()),
					ZkUtil.getSelectItemValueComboBox(cmbRECSTA), this.getLoginBean());
...
//		List<Row> rows = this.gridAccList.getRows().getChildren();
