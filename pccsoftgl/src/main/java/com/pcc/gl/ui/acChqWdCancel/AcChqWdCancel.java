package com.pcc.gl.ui.acChqWdCancel;
...
import com.pcc.gl.progman.ManAcChqWdCancel;
...
public class AcChqWdCancel extends FWinMenu {
	...
	public void onClick_btnFind() {
		try {
			...
			ManAcChqWdCancel.getDataQryForCancel(lst_gl_header, txtVOU_TYPE.getValue(), txtVOU_NO.getValue(),
		...
		Msg.confirm("ยืนยันการยกเลิก", "?", (event1) -> {
			if (Messagebox.Button.YES.equals(event1.getButton())) {
				try {
					...
					ManAcChqWdCancel.saveCancel(this.getLoginBean(), vou_type, vou_no);
					onClick_btnFind();
	...
	private RowRenderer getRowRenderer1() {
		...
		return (row, dat, index) -> {
			...
			btn_cancel.setLabel("ยกเลิก-ล้างเช็ค");
