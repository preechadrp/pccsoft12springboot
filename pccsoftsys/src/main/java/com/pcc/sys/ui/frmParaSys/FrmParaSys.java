package com.pcc.sys.ui.frmParaSys;
...
import com.pcc.sys.tbm.TbmFPARA_SYS;
import com.pcc.sys.tbo.TboFPARA_SYS;

public class FrmParaSys extends FWinMenu {
...
//						txtv1.addEventListener(Events.ON_CHANGE, (event) -> doOnClickRow(event)); //ไม่ ok ใน Iphone
	...
	private void show_data(String para_id, String para_desc) throws Exception {
		...
		TbmFPARA_SYS.getDataQry(lst_parameter, para_id, para_desc, this.getLoginBean());
	...
	private void doOnClickRow(Event event) {
		try {
			...
			TboFPARA_SYS fpc = new TboFPARA_SYS();

			fpc.setPARA_ID(sPARA_ID);

			if (TbfFPARA_SYS.find(fpc)) {
				fpc.setPARA_VALUE(txtv1.getValue());

				if (!TbfFPARA_SYS.update(fpc, "")) {
