...
public class FSearchPopup2 extends FWinUtil {
	...
	public FSearchPopup2(FSearchData fSearchData) {
		//constructor method
		System.out.println("FSearchPopup2");
		this.fSearchData = fSearchData;
		formInit();
	}
	...
	public void formInit() {
		...
		this.setSclass("popup-window");
		this.addEventListener(Events.ON_CANCEL, (event) -> onClose());

		divSearch.setSclass("mainGridLayout2");
		divSearch.setStyle("grid-template-columns: auto auto auto auto;");
		this.appendChild(divSearch);
		this.appendChild(new Separator());

		{
			Div div1 = new Div();
			div1.setStyle("text-align: center;");
			div1.appendChild(btnSearch);
			this.appendChild(div1);
		}

		btnSearch.setLabel(Labels.getLabel("comm.label.find2", "ค้นหาxxx")); //ไม่ต้องใส่คำว่า "labels." ข้างหน้าเหมือนใน zul
		btnSearch.addEventListener(Events.ON_CLICK, (event) -> searchData());

		listBox1.setMold("paging");
		listBox1.setPageSize(10);
		listBox1.setEmptyMessage("ไม่มีข้อมูล");
		this.appendChild(listBox1);

		Listhead lsH = new Listhead();
		lsH.setSizable(true);
		listBox1.appendChild(lsH);

		{
			Div div1 = new Div();
			div1.setStyle("text-align: center;background-color: #fff1c4;");
			this.appendChild(div1);

			Button btnOk = new Button("ตกลง");
			btnOk.addEventListener(Events.ON_CLICK, (event) -> onClick_btnOK());
			div1.appendChild(btnOk);

			Space sp1 = new Space();
			sp1.setSpacing("5px");
			div1.appendChild(sp1);

			Button btnClose = new Button("ออก");
			btnClose.addEventListener(Events.ON_CLICK, (event) -> onClose());
		...
		try {
			if (fSearchData.isShowCheckbox()) { //เลือกแบบ checkbox ส่งค่ากลับเป็น hasmap
				...
				if (lst_listitem.size() > 0) {
					...
					if (this.fSearchData.getEvent_callBack() != null) {
						this.fSearchData.getEvent_callBack().onEvent(new Event(Events.ON_CLICK, null, fSearchData.getReturnValueMapList()));
					} else {
				...
				if (item1 != null) {
					...
					if (fieldList.length > 1) { //คืนค่ามากกว่า 1 ฟิลด์จะใส่ค่ากลับเป็น hasmap แทน
						...
						if (item1.getAttribute(returnFieldName) != null) {
							...
							if (this.fSearchData.getEvent_callBack() != null) {
								this.fSearchData.getEvent_callBack().onEvent(new Event(Events.ON_CLICK, null, fSearchData.getReturnValue()));
							} else {
